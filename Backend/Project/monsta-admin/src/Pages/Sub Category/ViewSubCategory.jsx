...
export default function ViewCategory() {
  ...
  const changeStatus = () => {
    if(checkedValue.length > 0){
      axios.put('http://localhost:8000/api/admin/sub-categories/change-status', {
        id : checkedValue
      })
      .then((response) => {
        if (response.data._status == true) {
  ...
  const deleteRecords = () => {
    if(checkedValue.length > 0){
      if(confirm('Are you sure you want to delete ?')){
          axios.put('http://localhost:8000/api/admin/sub-categories/delete', {
            id : checkedValue
          })
          .then((response) => {
            if (response.data._status == true) {
                    ...
                    <tr>
                      <th scope="col" class="p-4">
                        <div class="flex items-center">
                          <input
                            onClick={ checkedAll }
                            checked={ subCategories.length == checkedValue.length ? 'checked' : '' }
